...
mod tests {
    ...
    #[test]
    fn test_format_file_type_and_permissions() {
        ...
        assert_eq!(format_file_type_and_permissions(&entry), ".rw-r--r--");
        ...
        assert_eq!(format_file_type_and_permissions(&entry), "drwxr-xr-x");

        // 3. Symlink
        let entry = create_entry(Specific::Symlink("target".into()), 0o777);
        assert_eq!(format_file_type_and_permissions(&entry), "lrwxrwxrwx");

        // 4. Block device
        let entry = create_entry(Specific::Block(DeviceData { major: 1, minor: 2 }), 0o660);
        assert_eq!(format_file_type_and_permissions(&entry), "brw-rw----");
        ...
        assert_eq!(format_file_type_and_permissions(&entry), "crw-------");
        ...
        assert_eq!(format_file_type_and_permissions(&entry), "srwxr-xr-x");
        ...
        assert_eq!(format_file_type_and_permissions(&entry), ".rwsr-xr-x");
        // Non-executable
        let entry = create_entry(
            Specific::Regular(RegularData {
                size: 10,
        ...
        assert_eq!(format_file_type_and_permissions(&entry), ".rwxr-sr-x");
        // Non-executable
        let entry = create_entry(
            Specific::Regular(RegularData {
                size: 10,
        ...
        assert_eq!(format_file_type_and_permissions(&entry), "drwxr-xr-t");
